...
import { AlertTriangle, Bot, ListTodo, User, ArrowUpRight } from 'lucide-react';
...
function statusLabel(task: Task, drift: boolean) {
  if (drift) return { text: 'Drifted', cls: 'badge-drift' };
  switch (task.status) {
    case 'in_progress':
  ...
  if (tasks.length === 0) {
    return (
      <EmptyState
        ...
        title="No tasks yet"
        description="Add a task from the toolbar to start tracking work."
      />
    );
  }
      ...
      {tasks.map((t, index) => {
        ...
        return (
          <Link
            ...
            className="group grid grid-cols-[2rem_1fr_auto_auto_1.25rem_auto_2rem_1.5rem] sm:grid-cols-[2rem_1fr_7rem_3rem_1.25rem_6rem_2rem_1.5rem] items-center gap-2 px-card py-3 hover:bg-surface-2/60 transition-colors"
          >
            {/* # */}
            ...
            {/* Title */}
            <span
              className={`font-medium truncate ${t.status === 'cancelled' ? 'text-fg-subtle line-through' : 'text-fg'}`}
            >
              {t.title}
            </span>
            ...
            <span className="hidden sm:flex items-center gap-1.5 min-w-0">
              {isAgent ? (
                <Bot className="h-3.5 w-3.5 text-agent shrink-0" aria-label="Agent" />
              ) : t.assignee ? (
                <User className="h-3.5 w-3.5 text-fg-subtle shrink-0" aria-label="Human" />
              ) : null}
              <span className="text-xs text-fg-muted truncate">{t.assignee || '—'}</span>
            </span>

            {/* Plan version */}
            <span className="hidden sm:inline-flex justify-center">
              <span className="badge badge-brand font-mono">v{t.boundPlanVersion}</span>
            </span>
            ...
            <span className="hidden sm:flex justify-center">
              {drift && (
                <AlertTriangle
                  className="h-3.5 w-3.5 text-drift"
                  aria-label="Task drifted from active plan"
                />
              )}
            </span>
            ...
            <span className="flex justify-center">
              <span className={`badge whitespace-nowrap ${label.cls}`}>{label.text}</span>
            </span>
            ...
            <span className="hidden sm:inline-flex justify-end">
              <ArrowUpRight
                className="h-3.5 w-3.5 text-fg-subtle group-hover:text-primary transition-colors"
                aria-hidden
              />
            </span>
          </Link>

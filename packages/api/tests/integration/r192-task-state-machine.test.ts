...
// ---------------------------------------------------------------
// 9. R-192 exit-permission gate (closes #1323 #1418)
// ---------------------------------------------------------------
//
...
//
// NOTE on file structure (closes #1418 / supersedes #1376): an earlier
// attempt to add this block landed a describe(...) call into the
// middle of section 6's `parkedAgentTask()` helper — specifically
// inside the `executionRun.create({ data: { ... } })` object literal
// — which left the test file unparseable. This block is appended at
// the END of the file as a self-contained section so future patches
// in the same family can be added without splitting unrelated
// helpers.

...
> ```
>
> - **AI 功能** —— 设置 `LLM_API_KEY`（AMD 内部 LLM）或 `ANTHROPIC_API_KEY`。**不设的话**，语义 plan diff、AI 完成验收、AI 冲突预测都会静默失效 —— 产品一半的核心能力。别跳过。
> - **共享主机 / 集群** —— 修改 `PG_PORT`，避免与同机其他用户冲突：
>   `PG_PORT=$(expr 15000 + $(id -u) % 1000)`
>
> 本机单人、不需要 AI？那就让 `ps-admin start` 自动生成默认 `.env`，直接跳到步骤 1。

```bash
# 1. 启动服务端。自动把 Node + Postgres 装到 .local-runtime/，
#    如果你跳过了上面的步骤，会自动从 .env.example 生成 .env，
#    跑 migration，全程不提问。
./bin/ps-admin start
